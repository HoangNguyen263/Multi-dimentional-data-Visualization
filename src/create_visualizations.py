...
def plot_weather_scatter(df: pd.DataFrame, outdir: str) -> str:
    ...
    city_colors = sns.color_palette("Set1", len(cities))
    ...
    city_handles = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor=city_color_map[c], markersize=10, label=c)
        for c in cities
    ]
    ...
    for val in precip_vals:
        size = np.interp(val, [0.0, precip_max], size_range)
        h = plt.scatter([], [], s=size, c='grey', alpha=0.65, label=f"{val:.1f}")
...
def plot_global_temp_heatmap(df: pd.DataFrame, outdir: str) -> str:
    ...
    # Replace columns with month name abbreviations for labeling on the plot
    pivot_df.columns = [months[col - 1] for col in pivot_df.columns]
